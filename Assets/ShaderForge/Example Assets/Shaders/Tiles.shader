...
/////// Normals:
                float2 node_1131 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_1131.rg, _Normal))).rgb;
...
///////// Gloss:
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_1131.rg, _Diffuse));
...
///////// Gloss:
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_1132.rg, _Diffuse));
                float gloss = (pow(node_138.r,2.0)*_Gloss);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
