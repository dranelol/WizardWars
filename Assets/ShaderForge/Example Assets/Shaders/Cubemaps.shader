...
/////// Normals:
                float2 node_1108 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_1108.rg, _Normal))).rgb;
...
////// Specular:
                ...
                float node_11 = 0.4;
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(node_1108.rg, _Specular));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_1109 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_1109.rg, _Normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(node_1109.rg, _Specular));
                float3 specularColor = float3(_Specular_var.r,_Specular_var.r,_Specular_var.r);
